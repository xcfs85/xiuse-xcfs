/*******************************************************************************
             数据库基类  (Oracle)                                   
             本类主要实现对数据库操作的各种方法              
             本类来源于互联网,大家有更好更新的版本时可以通知我们!!Aosy.net 
             {2016/12/11}                 
 * *****************************************************************************/
...
namespace Xiuse.DbUtility
{
	...
	public sealed class OracleHelper
	{
      ...
      /// <param name="commandType">执行类型[表名,存储过程,SQL语句]</param>
        ...
        /// <returns></returns>
        public static DataSet ExecuteDataset(string connectionString,CommandType commandType, int startRecord, int maxRecord, out int recordCount, string commandText, params OracleParameter[] parameters)
        {
            if (connectionString == null || connectionString.Length == 0) throw new ArgumentNullException("connectionString");
