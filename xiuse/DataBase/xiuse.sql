...
CREATE TABLE `xiuse_desk` (
  ...
  `TakeOut` bit(1) NOT NULL DEFAULT b'0' COMMENT '是否接受外卖（0，不接受外卖。1接受外卖）',
  `DeskDel` bit(1) NOT NULL DEFAULT b'1' COMMENT '0,已删除。1正常',
...
CREATE TABLE `xiuse_discount` (
  ...
  `DiscountState` bit(1) DEFAULT b'1' COMMENT '1,启用；0，禁用',
...
CREATE TABLE `xiuse_member` (
  ...
  `MemberState` bit(1) NOT NULL DEFAULT b'1' COMMENT '会员状态（0，禁用；1，启用；）',
...
CREATE TABLE `xiuse_user` (
  ...
  `ParentUserId` varchar(32) NOT NULL COMMENT '上级用户Id，顶级为空！',
  `OwnRestaurant` bit(1) DEFAULT b'0' COMMENT '餐厅所有权（0，无；1，所有；）',
